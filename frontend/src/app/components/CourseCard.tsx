...
function Wrapper({
  ...
  const base =
    "group block relative isolate rounded-2xl border border-white/10 bg-white/5 " +
    "hover:shadow-[0_10px_28px_rgba(0,0,0,.22)] transition-all overflow-visible"; // << important
...
export default function CourseCard(props: Course) {
  ...
  return (
    <Wrapper href={href} className={className}>
      {/* Media: chỉ overflow-hidden ở khung ảnh */}
      ...
      <div className="p-3.5 md:p-4">
        <h3 className="line-clamp-2 text-[15px] md:text-[16px] font-semibold leading-snug">
          {title}
        </h3>
        ...
        <div className="mt-2 flex items-center gap-2">
          <div className="text-[15px] md:text-[16px] font-bold">{price}</div>
          {originalPrice && (
      ...
      {/* subtle focus ring (không đẩy layout) */}
      <div className="pointer-events-none absolute inset-0 rounded-2xl ring-0 group-hover:ring-1 ring-white/10 transition" />
    </Wrapper>
  );
}
