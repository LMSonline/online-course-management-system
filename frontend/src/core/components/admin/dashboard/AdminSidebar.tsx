...
export function AdminSidebar({ stats }: Props) {
  ...
  return (
    <aside className={`w-64 ${darkMode ? "bg-[#0d1425]" : "bg-white"} border-r ${darkMode ? "border-gray-800" : "border-gray-200"} h-screen flex flex-col shrink-0`}>
      <div className={`p-6 border-b ${darkMode ? "border-gray-800" : "border-gray-200"} h-[73px] flex flex-col justify-center`}>
        <h1 className={`text-2xl font-bold ${darkMode ? "text-[#00ff00]" : "text-green-600"}`}>LMS Admin</h1>
        <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-600"} mt-1`}>Management Portal</p>
      </div>
